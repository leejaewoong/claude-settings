...
## Phase 0: MCP 연결 확인 (자동)

설정값 로드 후, Phase 1 진입 전에 Figma MCP 연결 상태를 자동으로 확인한다.
...
MCP_헬스체크:
  ...
  성공:
    설정: "MCP_STATUS = 'connected'"
    안내: "✓ Figma MCP 연결됨 ({사용자명}) — Phase 1을 시작합니다."

  실패:
    안내: |
      ⚠️ Figma MCP 연결을 확인할 수 없습니다.
      현재 상태에서는 HTML 생성까지만 가능하며, Figma 캡처(Phase 5)는 실행할 수 없습니다.

    진단_및_복구:
      설명: "에러 메시지에 따라 원인을 판별하고 적절한 복구 절차를 안내한다"

      케이스_1_인증_만료:
        판별: "에러에 'auth', 'token', 'unauthorized', '401', 'expired' 등이 포함"
        안내: |
          🔑 Figma 인증이 만료되었습니다. 재인증이 필요합니다.

          아래 절차를 따라주세요:
          1. 이 대화에서 `/mcp` 명령을 실행하세요
          2. 브라우저가 열리면 Figma OAuth 로그인을 완료하세요
          3. "Authentication successful" 메시지가 나타나면 완료입니다
          4. 완료 후 "재확인"을 선택하세요

      케이스_2_서버_미실행:
        판별: "에러에 'connect', 'ECONNREFUSED', 'not running', 'unavailable' 등이 포함"
        안내: |
          🔌 Figma MCP 서버가 실행되지 않았습니다.

          아래 절차를 따라주세요:
          1. /mcp 명령으로 figma 서버 상태를 확인하세요
          2. figma 서버가 목록에 없으면 플러그인 설정을 확인하세요:
             - ~/.claude/settings.json 의 enabledPlugins에 "figma@claude-plugins-official": true 확인
          3. 서버가 stopped 상태이면 /mcp에서 재시작하세요

      케이스_3_기타:
        판별: "위 케이스에 해당하지 않는 경우"
        안내: |
          ⚠️ 연결 실패 원인을 특정할 수 없습니다.

          일반적인 복구 절차:
          1. /mcp 명령으로 figma 서버 상태 확인
          2. 서버 재시작 시도
          3. 재시작 후에도 실패하면 /mcp에서 재인증 시도

    선택지: |
      1. 복구 후 재확인 — 위 절차를 완료한 뒤 MCP 연결을 다시 확인합니다
      2. HTML-only 모드로 진행 — Figma 캡처 없이 HTML만 생성합니다
      3. 중단

    처리:
      선택_1: "mcp__figma__whoami() 재호출 (최대 3회 반복 허용, 3회 실패 시 선택지 재제시)"
      선택_2: "MCP_STATUS = 'disconnected' 저장, Phase 1 진행"
      선택_3: "스킬 종료"
```
...
- HTML 생성 완료 후 Phase 5로 자동 진행 (Phase 5에서 MCP_STATUS에 따라 캡처 또는 HTML-only 완료)
...
/figma 실행
  │
  ├─ Phase 0: MCP 연결 확인
  │   ├─ whoami() 호출 → 성공 → MCP_STATUS=connected
  │   └─ 실패 → 복구 재시도(3회) / HTML-only 모드 / 중단
  │
  ├─ Phase 1: 대화로 메뉴 확정
  │   ├─ Q1: 1뎁스 선택 (13개 + 신규)
  │   ├─ Q2: 2뎁스 선택 (동적)
...
- **MCP_STATUS 변수**: Phase 0에서 설정. `connected`이면 Phase 5에서 Figma 캡처 실행, `disconnected`이면 Phase 5에서 HTML-only 모드로 마무리
...
- [x] ~~MCP 연결 사전 확인~~ → Phase 0 health-check + Phase 5 pre-capture validation 추가 완료
