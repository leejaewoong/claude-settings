...
분석_순서:
  ...
  4.7_프레임_배경색_필수:
    원칙: |
      Figma 캡처 시 프레임 배경이 반드시 채워져야 한다.
      .layout에 background 속성이 없으면 Figma 프레임이 투명하게 생성된다.
    필수_규칙:
      - ".layout에 반드시 background 지정 (var(--bg-primary, #1a1a22) 또는 명시적 hex)"
      - "body에도 background 지정 — 이중 보장"
      - "has-character 클래스 사용 시에도 background: #111115 명시"
      - "CSS 변수 fallback 필수 — var(--bg-primary, #1a1a22) 형태로 hex 폴백 포함"
    검증: ".layout과 body 모두 background 속성이 있는지 생성 후 확인"
  ...
  6_콘텐츠_완전성:
    ...
    반복_요소_전개:
      ...
      아이템_카드_필수_요소:
        - "등급/상태 태그 (무료, 보유, 등급명 — 국문)"
        - "아이템 이미지 영역 (placeholder 또는 이모지)"
        - "가격 표시 (미구매 시: 'C 120' 등)"
        - "아이템명 텍스트 (국문)"
        - "등급 텍스트 (일반, 희귀, 영웅, 전설, 신화 — 등급별 색상)"
        - "획득 완료 시: 오버레이 + ✓ 마크 + '보유' 텍스트"
...
```yaml
mock_data:
  원칙: "실제 PUBG 게임 데이터와 유사하게, 모든 텍스트는 국문(한글)으로 작성"
  언어: "KO — 디자인에 표시되는 모든 텍스트(버튼, 라벨, 메뉴, 상태, Mock 데이터)는 국문(한글)"

  플레이어명: "BETTER_Player01"  # 게이머태그는 영문 유지
  클랜명: "BETTERGROUND"  # 클랜명은 영문 유지
  레벨: 42
  티어: "다이아몬드 III"
