...
| `Bash(git ls-files:*)` | 인덱스/워킹트리의 파일 목록 조회 (`--others`, `--ignored` 등 옵션으로 추적/미추적 파일 필터링) |
...
| `Bash(echo:*)` | 텍스트 출력 |
| `Bash(sed:*)` | 스트림 편집기 (텍스트 치환) |
| `Bash(awk:*)` | 패턴 매칭 및 텍스트 처리 |
| `Bash(sort:*)` | 행 정렬 |
| `Bash(uniq:*)` | 중복 행 제거 |
| `Bash(tr:*)` | 문자 변환/삭제 |
| `Bash(cut:*)` | 행에서 특정 필드 추출 |
| `Bash(xargs:*)` | 표준 입력을 명령어 인자로 변환 |
| `Bash(tee:*)` | 표준 입력을 파일과 표준 출력에 동시 전달 |
| `Bash(diff:*)` | 파일 비교 |
| `Bash(touch:*)` | 빈 파일 생성 또는 타임스탬프 갱신 |
| `Bash(file:*)` | 파일 종류 판별 |
| `Bash(basename:*)` | 경로에서 파일명만 추출 |
| `Bash(dirname:*)` | 경로에서 디렉토리 부분만 추출 |
| `Bash(realpath:*)` | 심볼릭 링크 해석한 절대 경로 반환 |
| `Bash(du:*)` | 디스크 사용량 조회 |
| `Bash(stat:*)` | 파일 상태 정보 조회 |
| `Bash(taskkill:*)` | Windows 프로세스 강제 종료 (PID 또는 이미지 이름 기반) |
